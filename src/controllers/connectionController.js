...
const GET_FIELDS=["firstName", "lastName", "age", "bio", "skills", "gender",  "photoUrl" ]
...
const allConnectionsPendingController=async(req, res)=>{
   try {
    const toUserId=req.user.id;
    ...
    }).populate("fromUserId", GET_FIELDS).populate("toUserId", GET_FIELDS);

    const formattedData = allPendingConnection.map((connection) => {
        const isSent = connection.fromUserId._id.toString() === toUserId.toString();
    ...
    res.status(200).json({
        data:{
            code:200,
            success:true,
            message:"conncetion fetch successfuly",
            data:formattedData
        }
    })
   } catch (error) {
...
allConnectionsPendingController 
};
